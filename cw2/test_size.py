...
def reslice (Ori_Image,x = [1,0,0],y = [0,1,0],z = [0,0,1]):
    Ori_Image = (Ori_Image - np.amin(Ori_Image))/(np.amax(Ori_Image)-np.amin(Ori_Image))
...
# Do a 3d slice
Result1 = reslice(Filtered_slice,[1,0,0],[0,1,0],[0,0,1])
Result3 = np.nan_to_num(Result1,nan = 0)

#  reslice
Resliced_slice = reslice(array_of_data,[1,0,0],[0,1,0],[0,0,1])
# 2d Linear filter
sz = Resliced_slice.shape
Result2 = np.zeros(sz)
for i in range (sz[0]):
    Result2[i,:,:] = nonlinear_filter(Resliced_slice[i,:,:])
Result4 = np.nan_to_num(Result2,nan = 0)


# MSE:

Subtraction = Result4 - Result3

MSE = Subtraction ** 2
I = Subtraction

fig , ax = plt.subplots(3, 3)
ax[0,0].imshow(I[10,:,:], cmap='gray',origin='lower')
ax[0,1].imshow(I[100,:,:], cmap='gray',origin='lower')
ax[0,2].imshow(np.transpose(I[:,100,:]), cmap='gray',origin='lower')
ax[1,0].imshow(Result3[10,:,:], cmap='gray',origin='lower')
ax[1,1].imshow(Result3[100,:,:], cmap='gray',origin='lower')
ax[1,2].imshow(np.transpose(Result3[:,100,:]), cmap='gray',origin='lower')
ax[2,0].imshow(Result4[10,:,:], cmap='gray',origin='lower')
ax[2,1].imshow(Result4[100,:,:], cmap='gray',origin='lower')
ax[2,2].imshow(np.transpose(Result4[:,100,:]), cmap='gray',origin='lower')
